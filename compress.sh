...
find "$SRC_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.heic" -o -iname "*.arw" \) | while read -r FILE; do
  ...
  # Output is now .webp for maximum performance
  DEST_FILE="$TARGET_DIR/${SAFE_REL_PATH%.*}.webp"
  ...
  if [ ! -f "$DEST_FILE" ]; then
    echo "Compressing to WebP: $REL_PATH -> $SAFE_REL_PATH.webp"
    # -Z 1920 scales longest edge to 1920px. -s format webp applies WebP compression.
    # Note: sips supports webp on macOS Big Sur and later.
    sips -Z 1920 -s format webp "$FILE" --out "$DEST_FILE" >/dev/null 2>&1
  fi
done
