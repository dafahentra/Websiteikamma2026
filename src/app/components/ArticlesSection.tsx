      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          {ARTICLES.map((article, i) => (
            ...
            >
              {/* Image Placeholder */}
              <div className="w-full aspect-[4/3] bg-[#f0f2f5] mb-6 overflow-hidden">
                <div className="w-full h-full bg-[#e8eaed] group-hover:scale-105 transition-transform duration-500"></div>
              </div>

              {/* Text Content */}
              <p className="text-[#081C36]/50 text-sm md:text-base font-light mb-3">
                {article.date}
              </p>
              <h3 className="text-2xl font-bold text-[#081C36] mb-4 group-hover:text-[#081C36]/70 transition-colors duration-300">
                {article.title}
              </h3>
              <p className="text-[#081C36]/60 font-light leading-relaxed">
                {article.description}
              </p>
            </motion.div>
          ))}
